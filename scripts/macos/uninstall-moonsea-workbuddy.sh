...
if [[ "${INSTALL_ROOT:t}" != "MoonseaWorkBuddy" ]]; then
  echo "错误：拒绝卸载非 MoonseaWorkBuddy 目录：$INSTALL_ROOT" >&2
  exit 1
fi
...
if /usr/bin/pgrep -f "$INSTALL_ROOT/builds/Moonsea-WorkBuddy-" >/dev/null 2>&1; then
  echo "错误：请先关闭 WorkBuddy 月海版，再重新运行卸载脚本。" >&2
  exit 1
fi
...
if [[ -d "$INSTALL_ROOT" ]]; then
  if [[ "$REMOVE_USER_DATA" == "1" ]]; then
    ...
    /bin/rm -rf -- \
      ...
      "$INSTALL_ROOT/Start-Moonsea-WorkBuddy-macOS.command" \
