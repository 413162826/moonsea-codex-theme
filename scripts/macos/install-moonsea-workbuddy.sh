...
UPDATER_SOURCE="$SCRIPT_DIR/update-moonsea-workbuddy.sh"
APPLICATIONS_DIR="${MOONSEA_APPLICATIONS_DIR:-$HOME/Applications}"
DESKTOP_DIR="${MOONSEA_DESKTOP_DIR:-$HOME/Desktop}"
LAUNCHER_APP="$APPLICATIONS_DIR/WorkBuddy 月海版.app"
DESKTOP_LAUNCHER="$DESKTOP_DIR/WorkBuddy 月海版.app"
...
find_official_app() {
  ...
  if [[ -x /usr/bin/osascript ]]; then
    /usr/bin/osascript -e 'POSIX path of (choose application with prompt "请选择官方 WorkBuddy")' | sed 's:/$::'
    return
  fi
  fail "没有找到官方 WorkBuddy。请先安装并启动一次官方客户端。"
}

safe_remove_build() {
  local target="$1"
  case "$target" in
    "$BUILDS_ROOT"/Moonsea-WorkBuddy-*) /bin/rm -rf -- "$target" ;;
...
UPDATER_INSTALLED="$RELEASE_ROOT/scripts/macos/update-moonsea-workbuddy.sh"
...
is_valid_app "$SOURCE_APP" || fail "所选应用不是有效的官方 WorkBuddy：$SOURCE_APP"
...
if ! {
  ...
  /bin/cp "$UPDATER_SOURCE" "$RELEASE_STAGING/scripts/macos/update-moonsea-workbuddy.sh"
  /bin/chmod +x "$RELEASE_STAGING/scripts/macos/update-moonsea-workbuddy.sh"
...
/usr/bin/plutil -insert managerPort -integer 17322 "$TEMP_MANIFEST"
...
if [[ -z "${MOONSEA_SKIP_LAUNCHER_APP:-}" ]]; then
  ...
  /usr/bin/osacompile -o "$LAUNCHER_APP" "$SCRIPT_DIR/MoonseaWorkBuddyLauncher.applescript"
