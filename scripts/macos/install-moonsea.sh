...
THEME_VERSION="$(run_builder --theme-version | tail -n 1)"
[[ "$THEME_VERSION" =~ ^[a-f0-9]{12}$ ]] || fail "无法读取主题版本：$THEME_VERSION"

BUILD_NAME="Moonsea-Codex-$OFFICIAL_VERSION-$THEME_VERSION"
...
if [[ -d "$ACTIVE_BUILD" ]]; then
  if run_builder --verify "$ACTIVE_BUILD" >/dev/null; then
    NEEDS_BUILD=0
...
if [[ $NEEDS_BUILD -eq 1 ]]; then
  ...
  if ! run_builder --patch "$STAGING_BUILD"; then
