...
LAUNCHER_PATH="$INSTALL_ROOT/Start-Moonsea-WorkBuddy-macOS.command"
...
MANAGER_PORT="${MOONSEA_MANAGER_PORT:-17322}"
...
stop_moonsea_app() {
  local processes="$(/bin/ps ax -o pid=,command= | /usr/bin/grep -F "$BUILDS_ROOT/Moonsea-WorkBuddy-" | /usr/bin/grep -v grep || true)"
  if [[ -n "$processes" ]]; then
  ...
  for attempt in {1..50}; do
    local remaining="$(/bin/ps ax -o command= | /usr/bin/grep -F "$BUILDS_ROOT/Moonsea-WorkBuddy-" | /usr/bin/grep -v grep || true)"
...
restore_previous() {
  ...
  [[ ! -f "$ROLLBACK_ROOT/Start-Moonsea-WorkBuddy-macOS.command" ]] || {
    /bin/cp "$ROLLBACK_ROOT/Start-Moonsea-WorkBuddy-macOS.command" "$LAUNCHER_PATH"
...
[[ ! -f "$LAUNCHER_PATH" ]] || /bin/cp "$LAUNCHER_PATH" "$ROLLBACK_ROOT/Start-Moonsea-WorkBuddy-macOS.command"
