...
$stage = Join-Path $OutputDirectory "Moonsea-WorkBuddy-Windows-x64"
...
try {
    ...
    & $isccPath `
        "/DAppVersion=$version" `
        "/DPackageRoot=$stage" `
        "/DLauncherPath=$(Join-Path $launcherOutput 'MoonseaWorkBuddyLauncher.exe')" `
        "/O$setupOutput" `
        ".\installer\windows\WorkBuddy.iss"
