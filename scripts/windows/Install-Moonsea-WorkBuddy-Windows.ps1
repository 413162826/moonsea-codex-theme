...
if ([string]::IsNullOrWhiteSpace($InstallRoot)) {
    $InstallRoot = if ($env:MOONSEA_INSTALL_ROOT) { $env:MOONSEA_INSTALL_ROOT } else { Join-Path $env:LOCALAPPDATA "MoonseaWorkBuddy" }
}
...
$installedLauncherPath = Join-Path $InstallRoot "Start-Moonsea-WorkBuddy-Windows.ps1"
$launcherSourcePath = Join-Path $scriptRoot "Start-Moonsea-WorkBuddy-Windows.ps1"
...
$installedUpdaterPath = Join-Path $releaseRoot "scripts\windows\Update-Moonsea-WorkBuddy-Windows.ps1"
...
$official = Find-OfficialWorkBuddyApp
...
$buildName = "Moonsea-WorkBuddy-standard-$officialVersion-$themeVersion"
...
try {
    ...
    Copy-Item -LiteralPath $updaterSourcePath -Destination (Join-Path $updaterTargetDirectory "Update-Moonsea-WorkBuddy-Windows.ps1") -Force
...
if (-not $SkipShortcut -and -not $env:MOONSEA_SKIP_SHORTCUT) {
    ...
    $shortcut.IconLocation = "$(Join-Path $activeBuild 'WorkBuddy.exe'),0"
    $shortcut.Save()
}
