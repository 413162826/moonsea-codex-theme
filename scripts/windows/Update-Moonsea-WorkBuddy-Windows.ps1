...
function Stop-MoonseaApp([string]$BuildsRoot) {
    ...
    $processes = @(Get-CimInstance Win32_Process -Filter "Name = 'WorkBuddy.exe'" -ErrorAction SilentlyContinue | Where-Object {
...
function Wait-ForManager([string]$ExpectedVersion) {
    $managerPort = if ($env:MOONSEA_MANAGER_PORT) { [int]$env:MOONSEA_MANAGER_PORT } else { 17322 }
...
$launcherPath = Join-Path $InstallRoot "Start-Moonsea-WorkBuddy-Windows.ps1"
...
try {
    ...
    if (Test-Path -LiteralPath $launcherPath -PathType Leaf) {
        Copy-Item -LiteralPath $launcherPath -Destination (Join-Path $rollbackRoot "Start-Moonsea-WorkBuddy-Windows.ps1") -Force
    }
    ...
    $packageRoot = $packageRoots | Where-Object {
        Test-Path -LiteralPath (Join-Path $_ "scripts\windows\Install-Moonsea-WorkBuddy-Windows.ps1") -PathType Leaf
    } | Select-Object -First 1
    if ([string]::IsNullOrWhiteSpace($packageRoot)) { throw "The update package does not contain a Windows installer." }

    $installer = Join-Path $packageRoot "scripts\windows\Install-Moonsea-WorkBuddy-Windows.ps1"
    ...
    $rollbackLauncher = Join-Path $rollbackRoot "Start-Moonsea-WorkBuddy-Windows.ps1"
