...
if ([System.IO.Path]::GetFileName($InstallRoot) -ne "MoonseaWorkBuddy") {
    throw "Refusing to uninstall a non-MoonseaWorkBuddy directory: $InstallRoot"
}
...
$running = @(Get-CimInstance Win32_Process -Filter "Name = 'WorkBuddy.exe'" -ErrorAction SilentlyContinue | Where-Object {
...
if (Test-Path -LiteralPath $InstallRoot -PathType Container) {
    if ($RemoveUserData) {
        ...
        foreach ($name in @("builds", "releases", "updates", "install.json", "Start-Moonsea-WorkBuddy-Windows.ps1", "MoonseaWorkBuddyManager.exe", "MoonseaWorkBuddyManager.mjs", "manager.pid", "site")) {
